#!/usr/bin/perl
use RRDs;
my $logfile = '/home/black/my_code/systemgraphs/systemlog.html';
my $rrd = '/home/black/my_code/systemgraphs';
my $img = '/home/black/my_code/systemgraphs';
...
{
    my $in = `/sbin/ifconfig $_[0] |grep -v errors|grep "RX"|grep -Po 'bytes [0-9]+'|cut -d' ' -f2`;
    my $out = `/sbin/ifconfig $_[0] |grep -v errors|grep "TX"|grep -Po 'bytes [0-9]+'|cut -d' ' -f2`;
	chomp($in);
	chomp($out);
    ...
    print FH "<b>$_[0]</b> traffic in, out: $in, $out<br>\n";
    close(FH);
}
...
if ($debug eq "1") { print "memory -> free: $mem buffers: $buffers cached: $cached swap: $swap\n"; }
    open(FH, '>>', $logfile) or die $!;
    print FH "<B>MEMORY</b> free: $mem buffers: $buffers cached: $cached swap: $swap<br>\n";
    close(FH);
...
sub updatecpudata {
        ...
        open(FH, '>>', $logfile) or die $!;
        print FH "<B>CPU</b>: user: $user system: $system idle: $idle iowait: $io irq: $irq<br>\n";
        close(FH);
}
